...
class XorStr {
  public:
    constexpr XorStr(const _chartype (&str)[_length])
        : XorStr(str, std::make_index_sequence<length>()) {}
  ...
  private:
    ...
    constexpr XorStr(const _chartype (&str)[_length], std::index_sequence<_indices...>)
        : data{str[_indices]...}, encrypted(false) {}
...
class XorStr {
  public:
    constexpr XorStr(const _chartype (&str)[_length])
        : XorStr(str, std::make_index_sequence<_length>()) {}

    ~XorStr() {
        for (size_t t = 0; t < _length; t++) data[t] = 0;
    }
  ...
  private:
    template <size_t... _indices>
    constexpr XorStr(const _chartype (&str)[_length], std::index_sequence<_indices...>)
        : data{crypt(str[_indices], _indices)...}, encrypted(true) {}
